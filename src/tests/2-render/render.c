...
uint32_t elapsed_cycles(uint32_t start, uint32_t end) {
    // return (uint64_t)end + (0xffffffff * get_overflow_count()) - (uint64_t)start;
    return (end >= start) ? (end - start) : (0xFFFFFFFF - start + end + 1);
}

int has_overflow(uint32_t start, uint32_t end) {
...
void notmain(void) {
    ...
    if (rank == root) {
        ...
        // printk("start_one: %u\n", start_one);
        // printk("end_one  : %u\n", end_one);
        printk("num_cycles for %d: %x + 0xffffffff * %d\n", size, elapsed_cycles(start, end), get_overflow_count() - has_overflow(start, end));
